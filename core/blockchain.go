...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	if _, err := state.New(head.Root(), chainConfig.IsFirenze(head.Number()), bc.stateCache, bc.snaps); err != nil {
		...
		if currentBlock := bc.CurrentBlock(); currentBlock != nil && header.Number.Uint64() <= currentBlock.NumberU64() {
			...
			if newHeadBlock == nil {
				...
				for {
					...
					if _, err := state.New(newHeadBlock.Root(), bc.chainConfig.IsFirenze(newHeadBlock.Number()), bc.stateCache, bc.snaps); err != nil {
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		statedb, err := state.New(parent.Root, bc.chainConfig.IsFirenze(parent.Number), bc.stateCache, bc.snaps)
		if err != nil {
			return it.index, err
		}
		...
		if !bc.cacheConfig.TrieCleanNoPrefetch {
			if followup, err := it.peek(); followup != nil && err == nil {
				throwaway, _ := state.New(parent.Root, bc.chainConfig.IsFirenze(parent.Number), bc.stateCache, bc.snaps)
