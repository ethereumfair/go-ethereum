...
type Wallet struct {
	...
	deriveChain     dogecoin.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain dogecoin.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
